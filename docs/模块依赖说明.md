...
`useAppStore` 由四片组合：`projectSlice`（素材与 `config`）、`selectionSlice`、`canvasViewSlice`（画布索引、缩放、网格/标尺/安全边开关、吸附步长）、`layoutJobSlice`（`result`、自动排版、对齐等）。对外仍通过 `useAppStore` 单一入口订阅。
...
1. **自动排版**：`useAppStore.runAutoLayout` → 若存在 `electronAPI.runLayoutJob` 则 IPC `layout:run` → 主进程 `Worker` 执行 `executeLayoutJob`（内含 `buildLayoutValidationReport`）→ 可选写入 `layout_runs`；否则渲染进程 `executeLayoutJob`。手动拖拽后由 `withLayoutValidation` 刷新 `result.validation`。
